...
drop table if exists :results.drug_rollup;
create table :results.drug_rollup (
...
create index drdcidx on :results.drug_rollup (drug_concept_id);


insert into :results.drug_rollup 
select  distinct 
        de.drug_concept_id, 
        'ATC 3rd' as rollup_class,
        rc.concept_id as rollup_concept_id 
from :cdm.drug_exposure de
inner join :cdm.concept_ancestor ca on de.drug_concept_id = ca.descendant_concept_id 
inner join :cdm.concept rc on ca.ancestor_concept_id = rc.concept_id
...
drop table if exists :results.drug_exposure_rollup_pre;
create table :results.drug_exposure_rollup_pre as ( -- could be temporary table
  select  dr.*, 
          person_id, 
  ...
  from :cdm.drug_exposure de
  join :results.drug_rollup dr on de.drug_concept_id = dr.drug_concept_id
...
create index on :results.drug_exposure_rollup_pre (person_id, rollup_class, rollup_concept_id, drug_exposure_start_date, drug_exposure_id);

drop table if exists :results.drug_exposure_rollup cascade;
create table :results.drug_exposure_rollup as (
    select
        row_number() 
        ...
        drug_exposure_start_date -
          ...
          as days_from_latest,
        derp.*
    from :results.drug_exposure_rollup_pre derp
);

alter table :results.drug_exposure_rollup add primary key (person_id,rollup_class,rollup_concept_id,exp_num);
create index dersupidx on :results.drug_exposure_rollup (rollup_concept_id, days_supply);
create index dergapidx on :results.drug_exposure_rollup (rollup_concept_id, days_from_latest);
--create index derpeopleidx on :results.drug_exposure_rollup (person_id,rollup_concept_id,exp_num desc);

drop table if exists :results.drug_by_person_stats cascade;
create table :results.drug_by_person_stats as (
    ...
    from :results.drug_exposure_rollup der
    group by der.person_id, der.rollup_class, der.rollup_concept_id
  );

alter table :results.drug_by_person_stats_pre add primary key (person_id,rollup_class,rollup_concept_id);
create index drug_person_stats_pre_idx on :results.drug_by_person_stats_pre(rollup_class, rollup_concept_id);
create index drug_person_stats_pre_idx2 on :results.drug_by_person_stats_pre(rollup_concept_id, exp_count);

-- maybe just do this as needed?
create materialized view :results.drug_by_person_stats_ntile as (
    select
            ...
            dps.*
    from :results.drug_by_person_stats_pre dps
  );
create index drug_person_stats_idx on :results.drug_by_person_stats(rollup_class, rollup_concept_id);
  
drop table if exists :results.drug_rollup_stats;
create table :results.drug_rollup_stats as (
...
from :results.drug_by_person_stats  dbps
join :cdm.concept c on dbps.rollup_concept_id = c.concept_id
group by 1,2,3--,4
);
alter table :results.drug_rollup_stats add primary key (rollup_class, rollup_concept_id);
  ...
  from
      ...
      select generate_series as exp_numg, b.ntile
       from generate_series(1,
            (select max(exp_num) from :results.drug_exposure_rollup der
              where rollup_concept_id = $2)) 
...
select * from :results.DaySupplyByExpnum(4,21602054) ds
where ds.exp_or_gap_num = 3;

select * from :results.GapsByGapnum(4,21602054) gp
where gp.exp_or_gap_num = 3;

select * from :results.DaySupplyByExpnum(13,21602054) ds
left outer join :results.GapsByGapnum(13,21602054) gp 
  on ds.exp_or_gap_num = gp.exp_or_gap_num and ds.ntile = gp.ntile
  --and gp.exp_or_gap_num <= 3
where ds.exp_or_gap_num = 3; 

select distinct person_id, exp_num
    from :results.drug_exposure_rollup ds
...
from (
    ...
    from :results.drug_exposure_rollup ds
    left join :results.drug_exposure_rollup gp
...
from (
    ...
    from :results.drug_exposure_rollup gp
...
from drug_exposure_rollup der
join :cdm.concept c on der.rollup_concept_id=c.concept_id
join :cdm.drug_exposure de on der.drug_exposure_id = de.drug_exposure_id
group by 1,2,3
order by 1,2,3;
...
/*
create view :results.drug_exp_era_atc3_30 as (
  select sum(case when exp_num = 1 or days_from_latest > 30 then 1 else 0 end)
            over (partition by person_id, rollup_class, rollup_concept_id
              ...
              as era_num,
          dec.*
  from :results.drug_exposure_context dec
)


create table :results.atc3_30 as (
    select 
    ...
    join (
        ...
        from :results.drug_exposure_context
...
update :results.drug_exposure_extra dec
...
from (
    ...
    from :results.drug_exposure_extra
) deelag
...
create table :results.atc3_lookup
...
as (
    ...
    from (select distinct drug_concept_id from :cdm.drug_exposure) dids
    join :cdm.concept dc on dids.drug_concept_id = dc.concept_id
    ...
    left outer join (
        ...
        inner join :cdm.concept_ancestor ca on drug.concept_id = ca.descendant_concept_id 
...
create table :results.drug_exposure_stats_w_class
--with (location = user_db, distribution = hash(drug_concept_id))
as (
    ...
    from :cdm.drug_exposure de
    join :cdm.concept cde on de.drug_concept_id = cde.concept_id and cde.invalid_reason is null
    join :cdm.concept cdt on de.drug_type_concept_id = cdt.concept_id
